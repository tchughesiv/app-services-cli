...
const (
	...
	// Invalid decision cluster name
	ErrorMalformedDecisionClusterName ServiceErrorCode = 32
	...
	// Decision cluster name must be unique
	ErrorDuplicateDecisionClusterName ServiceErrorCode = 36
...
func GetAPIError(err error) (e decisclient.Error, ok bool) {
	var apiError decisclient.GenericOpenAPIError

	if ok = errors.As(err, &apiError); ok {
		errModel := apiError.Model()

		e, ok = errModel.(decisclient.Error)
	}
